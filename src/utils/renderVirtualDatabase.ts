 ...
 * @LastEditTime: 2022-06-24 14:54:20
...
export default (_options: OPTIONS_RESULT): DB_VIRTUAL_RESULT => {
    const database: DB_VIRTUAL_RESULT = {}, _db = _options.db || {};

    for (let _dbName in _db) {
        const _dbValue = _db[_dbName];
        database[_dbName] = mockjs.mock(typeof _dbValue === 'function' ? _dbValue({ mockjs }) : _dbValue);
    }
