 ...
 * @Date: 2022-06-24 14:54:13
 * @LastEditors: itmanyong itmanyong@gmail.com
 * @LastEditTime: 2022-06-24 16:19:43
...
export default (_options: OPTIONS_RESULT): APIS_VIRTUAL_RESULT => {
    const _apis = apis_virtual_result, { global, apis: apiModules } = _options;
    ...
    if (apiModules.length) {
        ...
        for (let mKey = 0, mLen = apiModules.length; mKey < mLen; mKey++) {
            ...
            for (let lKey = 0, lLen = apis.length; lKey < lLen; lKey++) {
                const api = apis[lKey];
                let url = `${global.prefix}${rest.prefix || ''}${api.url || ''}${'suffix' in rest ? rest.suffix : global.suffix}`;
                let method = (api.method || rest.method || global.method)?.toLowerCase() as METHOD_TYPE;
                ...
                const _api = { url, method, statusCode, pattern, keys, strict, timeout, render, meta };
